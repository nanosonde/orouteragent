...
see what the project does. It needs Docker; the E2E VM uses QEMU TCG and does
not require KVM or nested virtualization.
...
| `managed_domains` | *(empty)* | list of native packages the controller may manage: `network`, `dhcp`, `firewall`, `system`, `dropbear`; empty means no UCI is written |

`managed_domains` is opt-in on purpose: until a package is listed, matching
controller settings are validated and stored but never applied, so operator
...
- **SET**: acknowledged properly (per-key acks, echoed `configVersion`)
  and stored, then replayed on `GET`. Applying a setting to the router
  itself is opt-in per package through `managed_domains` and currently
  covers `ssh`, `wanIpv4`, `wanMac`, `lanDns`, `dnsCache`, `natPf`,
  `staticRouting`, and `timeSetting`; `wanBasicSetting` and LAN topology are
  still validated and stored only. The complete configuration surface is
  in [docs/gateway-config-reference.md](docs/gateway-config-reference.md);
  [docs/uci-config-mapping.md](docs/uci-config-mapping.md) gives the
  per-key OpenWrt implementation plan and
  [docs/config-implementation-status.md](docs/config-implementation-status.md)
  tracks the status of every option.
...
- **Most controller config is stored, not applied.** A `SET` from the
  controller is acknowledged and stored so the UI stays consistent. The
  base mappings listed above apply only when their owning package is listed
  in `managed_domains`; all other settings will *not* change router behavior.
