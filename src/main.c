...
int main(int argc, char **argv)
{
    ...
    ora_state_load(&st);
    /* Header seq must stay within a signed 32-bit range: the peer
     * deserializes MessageHeader.seq as a 32-bit signed integer and
     * discards messages whose seq overflows it. Seeding with epoch
     * milliseconds (masked) starts near 2.3e9 which is out of range and
     * makes every discovery announce undecodable. Epoch seconds masked
     * to 31 bits starts near 1.0e9 (roughly minute-resolution cycling)
     * and remains in range permanently. */
    ora_msg_seq_set((uint64_t)ora_now_s() & 0x3FFFFFFFull);
