    ...
    case ORA_MSG_SET_REQUEST: {
        ...
        if (req->body && json_object_is_type(req->body, json_type_object)) {
            json_object_object_foreach(req->body, key, val) {
                ...
                transform = ora_config_transform_find(key);
                ack_key = ora_config_transform_ack_key(key);
                ack_error_key = ora_config_transform_ack_error_key(key);
                ack = json_object_new_object();
                if (!ora_config_transform_validate(transform, cfg, val, error,
                                                   sizeof(error))) {
                    json_object_object_add(ack, ack_error_key, json_object_new_int(1));
                    json_object_object_add(ack, "errmsg", json_object_new_string(error));
                    json_object_object_add(resp, ack_key, ack);
                    ora_log(ORA_LOG_WARN, "SET seq %" PRId64 " key %s rejected: %s",
                            req->hdr.seq, key, error);
                    continue;
                }
                if (ora_uci_config_transform_is_managed(cfg, transform) &&
                    transform->status == ORA_CONFIG_TRANSFORM_APPLIED) {
                    apply_ok = ora_uci_config_apply(cfg, transform, val, error,
                                                    sizeof(error));
                }
                if (!apply_ok) {
                    json_object_object_add(ack, ack_error_key, json_object_new_int(1));
                    json_object_object_add(ack, "errmsg", json_object_new_string(error));
                    json_object_object_add(resp, ack_key, ack);
                    ora_log(ORA_LOG_WARN, "SET seq %" PRId64 " key %s not applied: %s",
                            req->hdr.seq, key, error);
                    continue;
                }
                json_object_object_add(ack, ack_error_key, json_object_new_int(0));
                json_object_object_add(resp, ack_key, ack);

                if (ora_uci_config_transform_is_managed(cfg, transform) &&
                    transform->status != ORA_CONFIG_TRANSFORM_APPLIED)
                    ora_log(ORA_LOG_INFO, "SET seq %" PRId64
                            " key %s staged for managed UCI domains",
                            req->hdr.seq, key);
    ...
    case ORA_MSG_GET_REQUEST: {
        ...
        if (req->body && json_object_is_type(req->body, json_type_object)) {
            json_object_object_foreach(req->body, key, val) {
                ...
                native = ora_uci_config_export(cfg, key);
                if (native) {
                    json_object *stored = b ? json_tokener_parse(json_object_get_string(b)) : NULL;

                    if (stored && json_object_is_type(stored, json_type_object)) {
                        json_object_object_foreach(native, native_key, native_value) {
                            json_object_object_add(stored, native_key,
                                                   json_object_get(native_value));
                        }
                    ...
                    ora_log(ORA_LOG_DEBUG, "GET seq %" PRId64
                            " key %s source UCI", req->hdr.seq, key);
                    json_object_object_add(resp_body, key, native);
                    continue;
                }
                if (b) {
