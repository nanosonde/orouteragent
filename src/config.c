...
static unsigned managed_domain_bit(const char *name)
{
    if (!strcmp(name, "network"))
        return ORA_CONFIG_DOMAIN_NETWORK;
    if (!strcmp(name, "dhcp"))
        return ORA_CONFIG_DOMAIN_DHCP;
    if (!strcmp(name, "firewall"))
        return ORA_CONFIG_DOMAIN_FIREWALL;
    if (!strcmp(name, "system"))
        return ORA_CONFIG_DOMAIN_SYSTEM;
    if (!strcmp(name, "dropbear"))
        return ORA_CONFIG_DOMAIN_DROPBEAR;
    return 0;
}

static unsigned uci_managed_domains(struct uci_option *o)
{
    ...
    if (o->type == UCI_TYPE_STRING) {
        unsigned bit = managed_domain_bit(o->v.string);

        if (!bit)
            ora_log(ORA_LOG_WARN, "unknown managed domain '%s' ignored",
                    o->v.string);
        return bit;
    }
    ...
    uci_foreach_element(&o->v.list, e) {
        unsigned bit = managed_domain_bit(e->name);

        if (!bit)
            ora_log(ORA_LOG_WARN, "unknown managed domain '%s' ignored", e->name);
        domains |= bit;
    }
    return domains;
}
    ...
    uci_foreach_element(&pkg->sections, e) {
        ...
        if (strcmp(s->type, "agent") == 0) {
            ...
            cfg->log_level = uci_int(cfg->log_level,
                ...
                cfg->managed_domains = uci_managed_domains(
                    uci_lookup_option(uctx, s, "managed_domains"));
...
bool ora_config_domains_enabled(const struct ora_config *cfg, unsigned domains)
{
    return cfg && domains && (cfg->managed_domains & domains) == domains;
}

int ora_config_ifname_port(const struct ora_config *cfg, const char *ifname)
{
    size_t i;

    if (!cfg || !ifname || !*ifname)
        return 0;
    for (i = 0; i < cfg->n_portmap; i++) {
        if (!strcmp(cfg->portmap[i].interface, ifname))
            return cfg->portmap[i].index;
    }
    if (!strcmp(cfg->default_wan_if, ifname))
        return 1;
    return !strcmp(cfg->default_lan_if, ifname) ? 2 : 0;
}
