...
enum ora_config_domain {
    ORA_CONFIG_DOMAIN_NETWORK = 1u << 0,
    ORA_CONFIG_DOMAIN_DHCP = 1u << 1,
    ORA_CONFIG_DOMAIN_FIREWALL = 1u << 2,
    ORA_CONFIG_DOMAIN_SYSTEM = 1u << 3,
    ORA_CONFIG_DOMAIN_DROPBEAR = 1u << 4,
};
...
struct ora_config {
    ...
    /* Native packages the controller may manage. Empty by default so an
     * installed agent never takes ownership of operator configuration. */
    unsigned managed_domains;
...
/* True when every bit in domains was explicitly enabled in UCI. */
bool ora_config_domains_enabled(const struct ora_config *cfg, unsigned domains);

/* Resolve an OpenWrt interface back to its emulated port number. */
