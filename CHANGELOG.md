...
## 2026-09-09T17:05:00+02:00

- Seed the message sequence counter from epoch seconds masked to 31 bits
  instead of epoch milliseconds masked to 32 bits. The previous seed started
  above the peer's signed 32-bit sequence range, so every discovery announce
  failed header decoding and unmanaged devices never appeared for adoption.
  Announces now decode on the first try and the pre-adopt handshake completes.

## 2026-09-09T16:42:27+02:00

- Add a dedicated E2E OpenWrt control NIC and isolated Docker network. Harness
  SSH now uses this interface, so controller LAN and WAN reload tests can run
  without losing their control connection.
- Run the E2E OpenWrt VM with QEMU TCG so nested virtualization and `/dev/kvm`
  are not required. Provision missing loop-device nodes in the privileged
  harness before offline image seeding.
- Refresh the SDK package source before each build so target-only smoke test
  artifacts are rebuilt from the current source tree.
- Add opt-in native UCI mappings for WAN IPv4 and MAC settings, static routes,
  local DNS records and cache settings, port forwards, and persistent NTP/time
  settings. The E2E smoke suite exercises network, DHCP, firewall, and system
  mappings on a real OpenWrt VM through a separate control interface.
- Start the bidirectional controller/UCI configuration foundation: add an
  opt-in `managed_domains` policy, a shared transform registry for the first
  native mapping set, SET payload shape validation, documented acknowledgement
  aliases, reverse port lookup, and the complete per-key implementation ledger.
- Apply the first bidirectional mapping: when `dropbear` is opted in, map the
  controller SSH enable and port settings to UCI, reload Dropbear with rollback
  on failure, and read those persistent values back for controller GET.
- Add `docs/uci-config-mapping.md`: an implementation-ready mapping for every
  controller SET key, covering current behavior, proposed native OpenWrt UCI
  or runtime owner, validation, model gates, dependencies, and safe apply
  guidance.
- Add `docs/gateway-config-reference.md`: the complete inventory of gateway
  configuration keys (SET/GET/INFORM) with wire payloads, response shapes and
  per-key target behavior, as the basis for full profile implementation.
...
## feat(config): apply base OpenWrt controller configuration

`806446b99237111e38efc6c0a2a8898b785bd776` - 2026-09-09T15:34:55+02:00

Implement the base UCI-backed controller mappings behind the existing
`managed_domains` opt-in policy.

- Apply WAN IPv4 DHCP, static and PPPoE settings plus primary WAN MAC overrides
  through netifd.
- Apply and reverse-export generated static routes, local DNS records, dnsmasq
  cache settings, firewall port forwards, and persistent timezone/NTP
  configuration.
- Use deterministic generated section names instead of unsupported UCI metadata
  options, preserving unrelated operator configuration.
- Validate controller booleans, IPv4 addresses, and port ranges before mutating
  UCI. Make optional UCI option deletion idempotent and make the UCI boundary
  report unavailable/non-native inputs accurately.
- Add a target-only transform smoke executable and an OpenWrt E2E recipe that
  validates real DHCP, firewall, and system UCI apply/export behavior while
  restoring the VM configuration.
- Update the implementation ledger, README, E2E guide, and changelog to match
  the current mappings and test coverage.

## feat(config): translate controller settings to UCI through a transform layer

`21beaa599d48fd4946da902d2cb53e008e042b52` - 2026-09-09T07:55:12+02:00

Add a bidirectional configuration layer shared by the controller config path
and the native UCI path, so a pushed setting can be translated in both
directions instead of only being remembered.

- `src/config/` holds the registry: one source file per native domain
  (network, dhcp, firewall, system, dropbear), shared validation helpers and
  a single UCI boundary that dispatches apply/export per key.
- New opt-in `managed_domains` UCI list, empty by default, decides which native
  packages the controller may manage. Keys outside an opted-in domain are
  validated and stored but never applied, so operator configuration is left
  alone.
- `ssh` is the first applied mapping: it writes the dropbear enable and port
  settings, reloads the service, restores the previous UCI values when that
  reload fails, and answers GET from UCI while keeping wire-only fields from
  stored state.
- `wanBasicSetting`, `wanIpv4`, `network`, `lanDns`, `dnsCache`, `natPf`,
  `staticRouting`, and `timeSetting` are registered with payload validation and
  stay stored until their transformers land.
- The acknowledgement contract now comes from the registry, keeping the
  request/ack renames and the `packageCapture` `errCode` spelling intact.
- `docs/config-implementation-status.md` tracks the status of every known
  configuration option; the README, the mapping document, the shipped UCI
  config, and the changelog follow the implementation.

## feat: inital commit

`76bf000d9d4293a66ef8202ea29bc8e8def0f54f` - 2026-09-09T07:16:08+02:00
