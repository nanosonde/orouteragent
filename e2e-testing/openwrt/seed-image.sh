...
CONTROL_VM_IP="${CONTROL_VM_IP:-198.18.0.50}"
CONTROL_PREFIX="${CONTROL_PREFIX:-24}"
...
set network.control.ipaddr='${CONTROL_VM_IP}'
set network.control.netmask='255.255.255.0'
delete network.control.gateway
delete network.control.dns
delete network.control.ip6assign
commit network
UCI

# The control network is trusted and is not managed by controller mappings.
uci -q batch <<UCI
add firewall zone
set firewall.@zone[-1].name='control'
add_list firewall.@zone[-1].network='control'
...
if [ -n "\$pkg" ]; then
    apk add --no-network --allow-untrusted "\$pkg" >/var/log/orouteragent-install.log 2>&1 || \
