...
mock. Four containers provide controller LAN, router WAN, and harness control
segments:

| Container | What it is |
|---|---|
| `ora-e2e-controller` | TP-L\*nk Om\*d\* Software Controller 6.2 (`mbentley/omada-controller`) |
| `ora-e2e-openwrt` | OpenWrt 25.12.5 x86-64 booted under QEMU TCG |
...
obtains an IPv4 address and default route from the ISP gateway. A third,
isolated control interface carries harness SSH only, so a controller-driven
LAN or WAN reload cannot interrupt E2E control.
...
the real GL-MT2500 target, because the VM is x86-64.
...
| `just e2e-ssh` | shell on the OpenWrt VM through its control interface |
| `just e2e-console` | VM serial console (telnet, ctrl-] to leave) |
| `just e2e-config-smoke` | build and test network, DHCP, firewall, and system UCI transforms; restores VM configuration afterward |
...
The E2E harness connects to the VM over an isolated third NIC (`eth2`) instead
of the controller LAN. The guest's `network.control` interface uses the static
address `198.18.0.50` by default; the container bridge uses `198.18.0.20`.
This network has no default route and is not a controller-managed interface.

| Variable | Default |
|---|---|
| `E2E_CONTROL_SUBNET` | `198.18.0.0/24` |
| `E2E_CONTROL_GATEWAY` | `198.18.0.1` |
| `E2E_CONTROL_IP` | `198.18.0.50` |
| `E2E_CONTROL_CONTAINER_IP` | `198.18.0.20` |
| `E2E_CONTROL_MAC` | `52:54:00:12:34:58` |
...
- The controller wizard is a genuine one-time step. Its database lives in
  ...
  after `e2e-destroy`.
- The OpenWrt container is privileged so it can mount the guest disk and
  rewrite its own networking into bridges for the guest interfaces. QEMU runs
  with software TCG and does not require nested virtualization or `/dev/kvm`;
  boot and test runs are consequently slower than KVM-backed runs.
